...
class BasicBacktester():
    ...
    def calculate_summary_statistics(self, scores):
        ...
        summary_statistics['gross_pnl_mean'] = round(scores['gross_pnl'].mean(), 4)
        summary_statistics['net_pnl_mean'] = round(scores['net_pnl'].mean(), 4)
        summary_statistics['gross_pnl_sum'] = round(scores['gross_pnl'].sum(), 4)
        summary_statistics['net_pnl_sum'] = round(scores['net_pnl'].sum(), 4)
        summary_statistics['gain_to_pain'] = round(scores[scores['net_pnl'] > 0]['net_pnl'].sum() / abs(scores[scores['net_pnl'] < 0]['net_pnl'].sum()), 4)
        summary_statistics['adjusted_expectancy'] = round(scores['net_pnl'].mean() / abs(scores[scores['net_pnl'] < 0]['net_pnl'].mean()), 4)
